...
/**
 * {@link OSGiVaadinServlet} that uses an {@link OsgiUIProvider} to configure
 * its {@link UI}.
 * <p>
...
public class OSGiVaadinServlet extends VaadinServlet {
	...
	static final Logger LOGGER = LoggerFactory.getLogger(OSGiVaadinServlet.class);

	private UIProvider uiProvider;
	private Configuration config;

	public OSGiVaadinServlet(UIProvider uiProvider, Configuration config) {
		...
		try {
			...
			service.setClassLoader(new ClassLoader() {
				...
				public Class<?> loadClass(String name) throws ClassNotFoundException {
					try {
						Class<?> loadClass = OSGiVaadinServlet.class.getClassLoader().loadClass(name);
						return loadClass;
					} catch (Exception e) {
						LOGGER.error("{}", e);
						throw e;
	...
	/**
	 * Local servlet service the carry the config and to add the {@link UIProvider}
	 * to the session.
	 */
